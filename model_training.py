        ...
        for j in range(no_iter_per_epoch):
            ...
            if j == no_iter_per_epoch - 1:
                ...
                for k in range(no_iter_per_epoch):
                    ...
                    train_ifft = torch.Tensor(
                        remove_frequency_component(x_train[batch * k:batch * (k + 1)], freq_min=freq_min,
                                                   freq_max=freq_max)
                    )
                    ...
                    train_ifft = train_ifft.to(device)
                    train_loss = lamb * loss_fn(train_out_mask, train_origin) + (1 - lamb) * loss_fn(train_out_filter,
                                                                                                     train_ifft)
                    train_loss = train_loss.cpu()
                    train_loss = train_loss.detach().numpy()
                ...
                for k in range(no_valid_loops):
                    ...
                    valid_ifft = valid_ifft.to(device)
                    valid_loss = lamb * loss_fn(valid_out_mask, valid_origin) + (1-lamb) * loss_fn(valid_out_filter, valid_ifft)
                    valid_loss = valid_loss.cpu()
                    valid_loss = valid_loss.detach().numpy()
                    valid_loss_list.append(valid_loss)
                ave_valid_loss = sum(valid_loss_list) / len(valid_loss_list)

                early_stopping(val_loss=ave_valid_loss, model=model)
...
def self_prediction_mask_and_frequency_(model, x_train, x_valid, lr=0.01, freq_min=51, lamb=0.5,
                                       freq_max=100, num_epochs=20, batch=64, patience=5,
                                       delta=0.01, loss_fn=nn.MSELoss(), shuffle=True, verbose=0):
    """
    Function to train MAEEG model by self-prediction with validation, filtering out frequencies.
    ...
    for epoch in range(num_epochs):
        ...
        for j in range(no_iter_per_epoch):
            out_filter = model(x_train[batch*j:batch*(j+1)])
            x_ifft = remove_frequency_component_(x_train[batch*j:batch*(j+1)], freq_min=freq_min, freq_max=freq_max)
            ...
            if j == no_iter_per_epoch - 1:
                ...
                for k in range(no_iter_per_epoch):
                    train_out_filter = model(x_train[batch * k: batch * (k + 1)])
                    train_ifft = remove_frequency_component_(x_train[batch * k:batch * (k + 1)], freq_min=freq_min,
                                                             freq_max=freq_max)
                ...
                for k in range(no_valid_loops):
                    valid_out_filter = model(x_valid[batch * k: batch * (k + 1)])
                    valid_ifft = remove_frequency_component_(x_valid[batch*k:batch*(k+1)], freq_min=freq_min,
                                                             freq_max=freq_max)
...
def self_prediction_mask_and_frequency_v2(model, x_train, x_valid, lr=0.01, lamb=0.5, freq_min=51,
                                          freq_max=100, n_components=10, random_state=0, num_epochs=20,
    ...
    for epoch in range(num_epochs):
        ...
        for j in range(no_iter_per_epoch):
            out_filter = model(x_train[batch*j:batch*(j+1)])
            x_ifft = torch.Tensor(ica_and_frequency_removal(x_train[batch*j:batch*(j+1)], freq_min=freq_min, freq_max=freq_max,
            ...
            if j == no_iter_per_epoch - 1:
                ...
                #     train_out_filter = model(x_train[batch * k: batch * (k + 1)])
                #     train_ifft = torch.Tensor(
                #         ica_and_frequency_removal(x_train[batch * k:batch * (k + 1)], freq_min=freq_min,
                #                                   freq_max=freq_max, n_components=n_components,
                ...
                for k in range(no_valid_loops):
                    valid_out_filter = model(x_valid[batch * k: batch * (k + 1)])
                    valid_ifft = torch.Tensor(
                        ica_and_frequency_removal(x_valid[batch*k:batch*(k+1)], freq_min=freq_min,
                                                  freq_max=freq_max, n_components=n_components, random_state=random_state)
                    )
                    valid_ifft = valid_ifft.to(device)
...
def remove_frequency_component_(x, freq_min, freq_max):
    """
    Function to remove frequencies out of the range.
    """
    ...
    if not torch.is_tensor(x):
        x = torch.Tensor(x)
    x = x.to(device)
    x_fft = torch.fft.rfft(x, axis=-1)

    x_fft[:, :, :freq_min] = 0
    x_fft[:, :, freq_max + 1:] = 0
    x_ifft = torch.fft.irfft(x_fft)
    return x_ifft
